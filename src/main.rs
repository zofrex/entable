...
fn main() {
    ...
    io::stdin().read_to_string(&mut input).expect("Something went wrong reading from stdin, somehow");
    ...
    for line in lines.clone() {
        let split_char = if divider_re.is_match(line) {
            "+"
        }
        else {
            "|"
        ...
        counts = this_line_counts.zip(counts.into_iter().chain(iter::repeat(0))).map(|(previous_max, current_size)| cmp::max(previous_max, current_size)).collect();
    }
